...
fn main() {
    let app = Application::new(|win_desc, state, window| {
        ...
        state.insert_stylesheet("src/cmd_palette_theme.css");
...
impl CommandPalette {
    pub fn new() -> Self {
        ...
        // This would be replaced by a file probably
...
impl BuildHandler for CommandPalette {
    ...
    fn on_build(&mut self, state: &mut State, entity: Entity) -> Self::Ret {
        ...
        self.search_box = Textbox::new("Type a command")
            .on_change(move |val| Event::new(SearchboxEvent::Changed(val.to_string())))
        ...
        // First option is currently selected
    ...
    fn on_event(&mut self, state: &mut State, entity: Entity, event: &mut Event) -> bool {
        ...
        if let Some(searchbox_event) = event.message.downcast::<SearchboxEvent>() {
            match searchbox_event {
                // Triggered when a character is types (or removed) from a textbox
                SearchboxEvent::Changed(val) => {
                    if event.target == self.search_box {
                        // Need to rewite this part to not use fuzzy_match *and* fuzzy_indices
...
impl EventHandler for SearchLabel {
    ...
    fn on_draw(&mut self, state: &mut State, entity: Entity, canvas: &mut Canvas<OpenGl>) {
        ...
        // TODO - create functions in tuix to make this a lot easier and less boilerplate
        ...
        // Stupid way, draw each glyph seperately (works though)
